...
static NSString *kNKUserDefaultsDidDownloadContentLengthKey = @"kNKUserDefaultsContentLengthKey";
...
static void nk_flushDidDownloadContentLength() {
    if ( ! kNKUserDefaultsDidDownloadContentLengthKey) return;
...
- (void)resume:(NSURL *)url {
    ...
    if (_currentItem == nil) {
        ...
        kNKUserDefaultsDidDownloadContentLengthKey = url.absoluteString.nk_md5;
        ...
        if (isExist) {
            ...
            if (_resumeFromBreakPoint && downloadContentLength > 0) {
                _currentItem.contentLength = nk_fetchDidDownloadContentLength();
...
didCompleteWithError:(nullable NSError *)error {
    ...
    if (_resumeFromBreakPoint) {
        nk_flushDidDownloadContentLength();
    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(download:didCompleteWithError:)]) {
