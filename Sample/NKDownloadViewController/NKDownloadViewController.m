...
@interface NKDownloadViewController () <HiNetworkDownloadTaskDelegate> {
    HiNetworkDownloadTask *_downloadTask;
    NSURL *_locationURL;
    NSURL *_downloadURL;
...
- (void)viewDidLoad {
    ...
    _downloadTask = HiNetworkDownloadTask.defaultTask;
...
- (void)didChangeReachability:(NSNotification *)notification {
    ...
    if ( ! item) return;
    HiNetworkReachabilityStatus status = item.intValue;
    switch (status) {
...
- (void)startDownload {
    ...
    if (_downloadTask.currentItem.state != HiNetworkDownloadItemStateDownloading &&
        _downloadTask.currentItem.state != HiNetworkDownloadItemStateFinished) {
        [self willRunning];
...
- (void)download:(HiNetworkDownloadItem *)item didCompleteWithError:(NSError *)error {
    if (error) {
